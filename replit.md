...
- **Windows Installer Bundle**: Production-ready multi-file Go agent in `/installer` directory. Files: `main.go` (entry point, branding, rotating logger), `config.go` (config.json + env var loading), `backend.go` (API client with retry/backoff), `telemetry.go` (CPU/RAM/process/network monitoring), `commands.go` (remote command handler with whitelist), `updater.go` (auto-update system), `service.go` (agent run loop with graceful shutdown). Supporting files: `config.json` (credentials template), `AegisAI360Agent.xml` (WinSW service wrapper), `installer.nsi` (NSIS installer with token prompt and branded BMP images), `AegisAI360-Header.bmp` / `AegisAI360-Banner.bmp` (24-bit NSIS images). Build with `go build -o agent.exe .` + `makensis installer.nsi`. Server URL: `https://aegisai360.com`.
