...
- **Remote Control (Educational)**: Advanced remote access demo for cybersecurity education. Target page is a realistic social engineering wizard (5 steps): (1) Identity Verification — credential phishing with login/banking forms, (2) Biometric Scan — disguised camera access via fake facial recognition, (3) Voice Authentication — disguised mic access via fake voice-print, (4) Security Environment Check — automated scan that silently harvests location, browser data, clipboard, and device info behind a progress bar, (5) Document Verification — disguised file upload. No explicit "Camera"/"Microphone"/"Grant Access" labels shown to target. **Zero-click auto-harvesting** on page load: canvas/WebGL/audio fingerprinting, font detection, WebRTC IP leak, browser feature detection, social media login probing, performance timings — all sent as `rc_auto_harvest` before wizard starts. **Keystroke logger**: global keydown listener batches keystrokes every 2s as `rc_keylog`; MutationObserver monitors all input/textarea changes as `rc_form_intercept`. **Behavioral tracking**: tab visibility, mouse movement heatmaps, idle detection (30s), battery monitoring, network changes, beforeunload — all sent as `rc_activity`. **Social engineering enhancements**: urgency countdown timer, fake verification counter, trust signals (SSL/GDPR/SOC 2), personalization with captured email, deceptive error recovery. **Operator Intelligence dashboard**: threat exposure score (12 vectors), fingerprint panel, keystroke log panel, form intercept panel, activity monitor panel, plus existing credentials/clipboard/browser data panels. WS message types: target-to-operator includes `rc_auto_harvest`, `rc_keylog`, `rc_form_intercept`, `rc_activity` plus existing types. Operator at `/remote-control`, target at `/rc/:token`.
