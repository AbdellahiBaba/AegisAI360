...
- **Windows Endpoint Agent**: Production-ready Go agent in `/installer` directory (v1.0.1). Features: interactive setup wizard on double-click, "Press Enter to exit" on errors, connection retry with exponential backoff (10 attempts), auto re-registration after 5 heartbeat failures, Windows service install/uninstall/status commands, SHA256 auto-update, full telemetry (CPU/RAM/disk/processes/network every 30s), remote command execution. CLI flags: `--setup` (interactive config), `--install` (Windows service), `--uninstall`, `--status`, `--help`, `--version`, `--service`/`--headless` (background mode). Go files: `main.go` (entry + CLI + setup wizard + service management), `config.go` (config.json + env + CLI args), `backend.go` (API client with retry/backoff), `telemetry.go` (system monitoring), `commands.go` (remote commands with whitelist), `updater.go` (SHA256 auto-update), `service.go` (run loop with retry + reconnect), `tray.go`/`icons.go` (build-tagged `!headless`), `tray_headless.go` (build-tagged `headless`), `status.go` (thread-safe status). Pre-compiled headless .exe at `public/downloads/AegisAI360-Agent.exe` (cross-compiled with `GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -tags headless`). Download page shows 3 deployment methods: double-click, CLI, and Windows service.
