...
- **Security Events Bulk Actions**: `PATCH /api/security-events/bulk` and `DELETE /api/security-events/bulk` endpoints with `bulkUpdateSecurityEventStatus` and `bulkDeleteSecurityEvents` storage methods. Frontend has checkbox selection, select-all, and bulk action bar (investigate, resolve, dismiss, delete).
- **Global Threat Map Redesign**: Enhanced SVG world map with dark cybersecurity aesthetic, animated attack arcs, hover tooltips, CSS keyframe pulse animations, glowing severity markers, animated attack counter, live feed sidebar, and progress bar country rankings.
- **Demo Data Removal**: `server/seed.ts` deleted. Network scanner demo fallback (`generateNetworkDevices`) removed — returns "no agents" message instead of fake data. All features use real data only.
...
- **Report Branding**: All PDF reports (`client/src/lib/reportGenerator.ts`) and HTML email reports (`server/reportScheduler.ts`) fully branded with AegisAI360 gold/amber (#D4AF37) + dark slate (#0f172a) color scheme. PDF reports feature a vector shield logo drawn with jsPDF primitives, branded headers with logo + "AEGISAI / CYBER DEFENSE PLATFORM" text + CONFIDENTIAL badge, gold-accented section titles, gold-tinted alternating table rows, and branded footers with aegisai360.com. Executive Summary and Compliance reports include a full-page cover page with centered logo, report title, generation date, and grid background. HTML email reports include inline SVG shield logo, dark header with gold branding, gold accent section borders, and branded footer.
