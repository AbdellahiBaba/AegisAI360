...
- **Remote Control (Educational)**: Permission-based remote access demo for cybersecurity education. Operators create sessions with expiry, generate shareable links. Targets open the link and grant browser permissions (camera, mic, location, device info, files) via native prompts. Data streams to operator in real-time via WebSocket/WebRTC. Session token validation on WS connect. Operator dashboard at `/remote-control`, target page at `/rc/:token`. Defense recommendations included.
