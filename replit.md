...
- **Windows Installer Bundle**: Production-ready multi-file Go agent in `/installer` directory with system tray GUI. Go files: `main.go` (entry point, --tray/--service mode detection, rotating logger), `config.go` (config.json + env var loading), `backend.go` (API client with retry/backoff), `telemetry.go` (CPU/RAM/process/network monitoring), `commands.go` (remote command handler with whitelist), `updater.go` (auto-update system), `service.go` (agent run loop with status tracking), `tray.go` (system tray icon + menu, build-tagged `!headless`), `icons.go` (embedded ICO generation, build-tagged `!headless`), `tray_headless.go` (stub for headless builds, build-tagged `headless`), `status.go` (thread-safe status struct shared between service and tray). Supporting files: `config.json`, `AegisAI360Agent.xml` (WinSW), `installer.nsi` (NSIS with token prompt, Start Menu + Desktop shortcuts, auto-start on login), `AegisAI360-Header.bmp` / `AegisAI360-Banner.bmp`. Pre-compiled headless Windows .exe at `public/downloads/AegisAI360-Agent.exe` (cross-compiled with `GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -tags headless`). Full tray build (requires CGo on Windows): `go build -ldflags "-H windowsgui" -o agent.exe .` + `makensis installer.nsi`. Download served via Express static at `/downloads/AegisAI360-Agent.exe`. Usage: `AegisAI360-Agent.exe https://aegisai360.com agt_YOUR_TOKEN`. Ping endpoint: `GET /api/agent/ping` (no auth).
- **Files**: `server/agentApi.ts`, `client/src/pages/endpoints.tsx`, `client/src/pages/download-agent.tsx`, `client/src/pages/agent-terminal.tsx`, `installer/*.go`, `installer/installer.nsi`, `installer/AegisAI360Agent.xml`, `installer/config.json`
