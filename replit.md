...
- **Windows Installer Bundle**: Production-ready multi-file Go agent in `/installer` directory with system tray GUI. Go files: `main.go` (entry point, --tray/--service mode detection, rotating logger), `config.go` (config.json + env var loading), `backend.go` (API client with retry/backoff), `telemetry.go` (CPU/RAM/process/network monitoring), `commands.go` (remote command handler with whitelist), `updater.go` (auto-update system), `service.go` (agent run loop with status tracking), `tray.go` (system tray icon + menu: Open Dashboard, Test Connection, View Logs, Restart, Exit), `icons.go` (embedded ICO generation for connected/disconnected states), `status.go` (thread-safe status struct shared between service and tray). Supporting files: `config.json`, `AegisAI360Agent.xml` (WinSW), `installer.nsi` (NSIS with token prompt, Start Menu + Desktop shortcuts, auto-start on login), `AegisAI360-Header.bmp` / `AegisAI360-Banner.bmp`. Build: `go build -ldflags "-H windowsgui" -o agent.exe .` + `makensis installer.nsi`. Server: `https://aegisai360.com`. Ping endpoint: `GET /api/agent/ping` (no auth).
