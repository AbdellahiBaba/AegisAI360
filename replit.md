...
- **AegisAI360 AI Agent**: Standalone cybersecurity AI agent module at `/ai-agent`. Backend module at `server/aegisAgent.ts` with comprehensive system prompt covering threat modeling, vulnerability analysis, secure coding, malware education, incident response, and security architecture. Features: streaming chat with conversation persistence (`agent_conversations`/`agent_messages` tables), code block extraction with syntax highlighting and copy/download, secure code generation endpoint, file packaging/download. Capability manifest and quick-action prompts. Routes registered via `server/aegisAgentRoutes.ts`. Programmatic interface: `streamAgentChat()`, `generateSecureCode()`, `generateCodePackage()`, `AGENT_CAPABILITIES`.
