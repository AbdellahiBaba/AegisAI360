...
- **Remote Control (Educational)**: Advanced remote access demo for cybersecurity education. 8 permission types: camera, mic, location, device info, files, credential harvesting (phishing overlay with login/banking forms), clipboard capture, and browser data extraction (cookies, localStorage, sessionStorage, plugins, history). Operator dashboard has 5-tab layout (Control, Live Feed, Intelligence, Files, Events) with remote permission requests, camera/mic toggle, video/audio recording, screenshots, credential viewer, browser data viewer, clipboard log, and event log with bulk actions (select all, export JSON, copy, clear). Target page shows no camera/mic previews (stealth mode). Role-based WebSocket relay validation. Operator at `/remote-control`, target at `/rc/:token`.
