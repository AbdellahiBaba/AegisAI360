...
- **Public Pages**: Landing page (`/`) showcases all platform capabilities with "How It Works" flow and trust/compliance badges. Features page (`/features`) has 22 detailed feature cards in 5 categories. Pricing page (`/pricing`) shows feature comparison table across all tiers. About page (`/about`) with team and mission info. Contact page (`/contact`), FAQ page (`/faq`), and Security Trust Center (`/security`) pages available. All public pages fully internationalized (EN/AR).
- **Cybersecurity Animations**: Landing page includes CyberThreatFeed (live threat ticker), CyberTerminal (scrolling security logs), and CyberNetwork (animated network topology) components. Features page includes CyberRadar (sonar sweep).
